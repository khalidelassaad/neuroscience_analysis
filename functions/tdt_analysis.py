...
def fp_preprocess(dir_extracted: str, dir_processed: str, log_fp: pd.DataFrame,
    ...
    if key_files.empty:
        print("[-] extracted_to_processed: WARNING: No files in dir_extracted")
        return
    ...
    subjects_string = '|'.join(data_in_log_fp)
    key_files = to_frame(
        key_files[
    ...
    if overwrite == 0:
        ...
        if key_files.empty:
            print("[-] extracted_to_processed: No new data to process")
            print("")
            print("[-] extracted_to_processed: To overwrite data, set overwrite = 1")
            return
    ...
    for n_file in range(key_files.shape[0]):
        ...
        if 'blockname' not in streams_info.columns:
            missing_headers.append(f"{streams_path} (streams_info)")
        ...
        if not streams_data_smoothed_fitted.empty:
            ...
            if flag_events_tdt_epocs:
                ...
                if events.empty:
                    ...
                    if events.get('event_id_char') is not None and np.sum(~events['event_id_char'].isna()) > 0:
...
if __name__ == "__main__":
    main(
        '/home/khalid/Downloads/path/frequency_data/4021L/extracted',
        '/home/khalid/Downloads/path/frequency_data/4021L/processed',
        '/home/khalid/Downloads/path/frequency_data/4021L/log_cera_data_4021L.csv'
    )
